      ...
      child: GetBuilder<ForYouController>(
        builder: (ForYouController controller) {
          final List<String> initialInViewIds = [
            if (controller.videosList.length > 0) controller.videosList.values.elementAt(0).id,
            if (controller.videosList.length > 1) controller.videosList.values.elementAt(1).id,
          ];
  ...
  Widget _buildBody(BuildContext context) {
    ...
    return SliverPadding(
      ...
      sliver: _buildVideoGrid(MediaQuery.of(context).size.width,
          controller.isLoading, controller.videosList.values.toList()),
    );
  }
